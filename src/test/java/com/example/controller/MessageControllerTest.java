...
class MessageControllerTest {
    ...
    void sendMessage_withValidRequest_shouldReturnAccepted() throws Exception {
        SendMessageRequest request = new SendMessageRequest();
        request.setRecipientId(UUID.randomUUID());
        request.setContent("Hello");

        when(messageService.sendMessage(any(UUID.class), any(UUID.class), eq("Hello"))).thenReturn(new SendMessageResponse());
